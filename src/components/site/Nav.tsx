  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled || open
          ? "bg-[#070b19]/70 backdrop-blur-xl border-b border-white/5 py-4"
          : "bg-transparent py-6"
      }`}
    >
      <div className="mx-auto max-w-[1240px] px-5 sm:px-8 flex items-center justify-between">
        ...
        >
          <div className="h-8.5 w-8.5 rounded-lg bg-gradient-primary p-[1.5px] flex items-center justify-center shadow-glow transition-transform duration-300 group-hover:scale-105">
            <div className="h-full w-full bg-[#070b19] rounded-[6px] flex items-center justify-center">
        ...
        <div className="flex items-center gap-3">
          <a
            ...
            download
            className="hidden sm:inline-flex items-center gap-2 rounded-lg bg-gradient-primary text-primary-foreground px-5 py-2.5 text-xs font-semibold shadow-glow hover:opacity-90 transition active:scale-95"
          >
      ...
      {open && (
        <div className="md:hidden fixed inset-x-0 top-16 bottom-0 z-40 bg-[#070b19]/95 backdrop-blur-2xl animate-fade-in border-t border-white/5">
          <nav className="px-6 py-10 flex flex-col gap-6 h-full justify-between">
            <div className="flex flex-col gap-2">
              {links.map((l) => (
                <Link
                  ...
                  to={l.to}
                  onClick={() => setOpen(false)}
                  className="font-serif-display text-[32px] font-bold py-3 text-foreground hover:text-accent border-b border-white/5 flex justify-between items-center"
                >
                  {l.label}
            ...
            <a
              ...
              download
              onClick={() => setOpen(false)}
              className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-primary text-primary-foreground py-4 text-sm font-bold shadow-glow"
            >
