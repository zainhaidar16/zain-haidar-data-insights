      ...
      <div className="section-container">
        ...
        {loading && (
          <div className="grid md:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-[#FFFFFF] rounded-[24px] p-6 animate-pulse border border-[#E8E8ED]"
              >
        ...
        {error && !loading && (
          <div className="p-6 bg-red-50 border border-red-200 rounded-[24px] flex items-start gap-3.5 max-w-2xl mx-auto">
            ...
            <div>
              <h4 className="font-bold text-red-800 text-sm">Failed to Load Guides</h4>
        ...
        {!loading && !error && posts.length === 0 && (
          <div className="py-16 text-center max-w-md mx-auto">
            <div className="h-12 w-12 rounded-full bg-[#FFFFFF] border border-[#E8E8ED] flex items-center justify-center mx-auto mb-4">
              <BookOpen className="h-5 w-5 text-[#0071E3]" />
            </div>
            <h4 className="font-bold text-[#1D1D1F] text-sm mb-1">No guides published yet</h4>
            ...
            {posts.slice(0, 3).map((post, i) => {
              ...
              return (
                <motion.article
                  ...
                  className="bg-[#FFFFFF] rounded-[24px] border border-[#E8E8ED] overflow-hidden shadow-none hover:border-[#0071E3]/30 hover:shadow-md transition-all duration-300 flex flex-col group"
                >
                  {post.cover_url ? (
                    ...
                    <div className="aspect-[16/9] bg-[#F5F5F7] border-b border-[#E8E8ED] flex items-center justify-center">
                      <div className="w-[70%] rounded-2xl border border-[#E8E8ED] bg-[#FFFFFF] p-4">
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-[#0071E3] flex items-center gap-1">
                        <Tag className="h-3 w-3" />
                        {post.category ?? "Article"}
                      </span>
                      <span className="text-[10px] font-semibold text-[#86868B] flex items-center gap-1">
                    ...
                    <h3 className="font-bold text-[#1D1D1F] text-[16px] leading-snug mb-2 group-hover:text-[#0071E3] transition-colors line-clamp-2">
                      {post.title}
                    </h3>
                    ...
                    {tags.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-4">
                        {tags.slice(0, 3).map((tag) => (
                          <span
                            key={tag}
                            className="px-2 py-0.5 rounded-full bg-[#F5F5F7] border border-[#E8E8ED] text-[9px] font-medium text-[#86868B]"
                          >
                            #{tag}
                          </span>
                        ))}
