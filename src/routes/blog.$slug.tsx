  ...
  return (
    <main className="min-h-screen bg-[var(--site-bg)] text-[var(--text-soft)]">
      ...
      <article className="pb-16 pt-28 md:pt-32">
        {post.cover_url && (
          <div className="mx-auto mb-8 max-w-[860px] px-6">
            <div className="aspect-[21/9] overflow-hidden rounded-2xl bg-[var(--site-bg-muted)]">
        ...
        <header className="mx-auto max-w-[680px] px-6">
          <Link
            to="/blog"
            className="mb-5 inline-flex items-center gap-2 text-xs font-normal uppercase tracking-[0.1em] text-[var(--text-muted)] transition-colors hover:text-[var(--text-main)]"
          >
          ...
          {post.category && (
            <p className="mb-3 text-xs font-normal uppercase tracking-[0.1em] text-[var(--purple)]">
              {post.category}
            </p>
          )}

          <h1 className="text-3xl font-bold leading-[1.1] tracking-normal text-[var(--text-main)] sm:text-4xl md:text-[44px]">
            {post.title}
          </h1>
          ...
          {post.excerpt && (
            <p className="mt-6 text-lg font-normal leading-relaxed text-[var(--text-muted)]">
              {post.excerpt}
            </p>
          )}

          <div className="mt-6 flex flex-wrap items-center gap-2.5 border-y border-[var(--border)] py-3 text-sm">
        ...
        <div className="mx-auto mt-10 grid max-w-5xl grid-cols-1 gap-8 px-6 xl:grid-cols-[210px_minmax(0,680px)_80px]">
          <aside className="hidden xl:block">
            {headings.length > 0 && (
              <nav className="sticky top-24 space-y-2 text-[13px]" aria-label="Table of contents">
                <p className="text-xs uppercase tracking-[0.1em] text-[var(--text-muted)]">On this page</p>
                <div className="space-y-1.5 border-l border-[var(--border)] pl-4">
          ...
          <div className="max-w-[680px] xl:col-start-2">
            <div className="prose max-w-none text-[18px] font-normal leading-[1.7] text-[var(--text-soft)] [&_h1]:text-[var(--text-main)] [&_h2]:text-[var(--text-main)] [&_h3]:text-[var(--text-main)] [&_h4]:text-[var(--text-main)] [&_li]:marker:text-[var(--purple)] [&_ol]:list-decimal [&_ol]:space-y-2.5 [&_ol]:pl-6 [&_p]:mb-5 [&_p]:leading-[1.7] [&_strong]:font-semibold [&_strong]:text-[var(--text-main)] [&_ul]:list-disc [&_ul]:space-y-2.5 [&_ul]:pl-6">
              <ReactMarkdown
                components={{
                  ...
                  p: ({ node, ...props }) => (
                    <p className="mb-5 text-[var(--text-soft)] leading-[1.7]" {...props} />
                  ),
                  strong: ({ node, ...props }) => (
                    <strong className="font-semibold text-[var(--text-main)]" {...props} />
                  ),
                  ul: ({ node, ...props }) => (
                    <ul className="mb-5 list-disc space-y-2.5 pl-6" {...props} />
                  ),
                  ol: ({ node, ...props }) => (
                    <ol className="mb-5 list-decimal space-y-2.5 pl-6" {...props} />
                  ),
                  li: ({ node, ...props }) => (
                    <li className="leading-[1.7] text-[var(--text-soft)]" {...props} />
                  ),
                  code: ({ node, ...props }) => (
                    <code
                  ...
                  pre: ({ node, ...props }) => (
                    <pre
                      className="mb-5 overflow-x-auto rounded-2xl border border-[var(--border)] bg-[var(--site-bg-muted)] p-4 font-mono text-sm leading-relaxed text-[var(--text-main)] shadow-sm"
                      {...props}
                    />
                  ),
                  blockquote: ({ node, ...props }) => (
                    <blockquote
                      className="mb-5 rounded-r-lg border-l-4 border-[var(--purple)] bg-[var(--site-bg-soft)] py-3 pl-5 pr-4 italic text-[var(--text-soft)]"
                      {...props}
                    />
                  ),
                }}
...
function BlogArticleSkeleton() {
  return (
    <main className="min-h-screen bg-[var(--site-bg)] text-[var(--text-soft)]">
      <Header />
      <div className="mx-auto max-w-[860px] px-6 pt-28 md:pt-32">
        <div className="aspect-[21/9] animate-pulse rounded-2xl bg-[var(--site-bg-muted)]" />
      </div>
      <div className="mx-auto max-w-[680px] space-y-5 px-6 py-10">
